...
# Override cd — cannot leave project directory
cd() {
  local target
  if [ $# -eq 0 ]; then
    target="$VAJRA_PROJECT"
  elif [ "$1" = "-" ]; then
    target="$OLDPWD"
  elif [ "${1:0:1}" = "/" ]; then
    target="$1"
  ...
  if [ "${target#"$VAJRA_PROJECT"}" = "$target" ]; then
    _vajra_log "cd" "Cannot leave project directory: $VAJRA_PROJECT"
    return 1
  fi
  builtin cd "$@"
}
...
_vajra_allowlist="^(ls|cat|head|tail|wc|file|stat|find|tree|touch|mkdir|cp|mv|rm|chmod|ln|grep|sed|awk|sort|uniq|tr|cut|paste|xargs|echo|printf|uname|whoami|id|pwd|date|env|which|type|npm|pnpm|yarn|node|make|gcc|test|true|false|read|export|source|alias|unalias|less|more|diff|patch|tee|df|du|ps|free|uptime|hostname|arch|trap|set|shopt|builtin|command)$"
...
# Welcome message
echo ""
echo "Sandboxed shell — confined to: /mnt/data/Repos/vajra"
