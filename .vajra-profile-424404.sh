...
export PATH="/home/mrugesh/.local/bin:/home/mrugesh/flutter/bin:/home/mrugesh/.opencode/bin:/home/mrugesh/.pyenv/shims:/home/mrugesh/.pyenv/bin:/home/mrugesh/Flutter/flutter/bin:/tmp/.mount_T3-Codw985nB:/home/mrugesh/.local/share/pnpm/bin:/home/mrugesh/.nvm/versions/node/v24.16.0/bin:/home/mrugesh/.cargo/bin:/usr/local/bin:/usr/bin:/bin:/usr/games:/usr/local/games:/snap/bin:/usr/lib/android-sdk/cmdline-tools/latest/bin:/usr/lib/android-sdk/platform-tools:/usr/local/go/bin"
...
trap '
  ...
  # Block cd beyond project directory
  if echo "$_vajra_cmd" | grep -qE "^cd$"; then
    _vajra_target="$(cd "$VAJRA_PROJECT" 2>/dev/null && cd "${_vajra_line#cd }" 2>/dev/null && pwd)"
    if [ -n "$_vajra_target" ] && [ "${_vajra_target#"$VAJRA_PROJECT"}" != "$_vajra_target" ]; then
