...
export function placeFootprints(
  ...
  const others = sorted.filter((c) => !isIC(c) && !isConnector(c))
...
function isIC(comp: SchematicComponent): boolean {
  const val = comp.value.toLowerCase()
  const sym = comp.symbol.toLowerCase()
  return val.includes('ic') || val.includes('mcu') || val.includes('cpu') ||
    val.includes('stm32') || val.includes('atmega') || val.includes('esp32') ||
    sym.includes('ic') || sym.includes('mcu')
}

function isConnector(comp: SchematicComponent): boolean {
  const val = comp.value.toLowerCase()
  const sym = comp.symbol.toLowerCase()
