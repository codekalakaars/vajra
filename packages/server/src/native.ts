// The single place this package touches `vajra-core` — and deliberately
...
// `worker/sandboxed-worker.mjs` — which requires `vajra-core` on its own,
...
//
// Return types are annotated with @codekalakaars/protocol's hand-mirrored shapes
// rather than cast — TypeScript's structural typing checks the two agree,
// so this is real verification, not decoration. If vajra-core's shape ever
...
import type { PermissionsConfig, ProjectFileEntry } from '@codekalakaars/protocol'
